...
(function () {
  ...
  function next() {
    ...
    if (nextReal >= total) {
      ...
      setTimeout(function () {
        // Silent snap back to the real first slide. Suppress *all* slide
        // transitions (width, filter, transform on each .strip-image) during
        // the reposition, otherwise the real slide visibly "grows in" from
        // its neighbor state and breaks the infinite-loop illusion.
        ...
        requestAnimationFrame(function () {
          ...
          // Reset the interval so pacing stays consistent after the wrap
          clearInterval(timer);
          startLoop();
        });
      }, 620);
